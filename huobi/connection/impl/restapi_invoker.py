...
def call_sync(request, is_checked=False):
    if request.method == "GET":
        ...
        response = session.get(request.host + request.url, headers=request.header, proxies=proxy)
        ...
        response = session.post(request.host + request.url, data=json.dumps(request.post_body), headers=request.header, proxies=proxy)
        dict_data = json.loads(response.text, encoding="utf-8")
        # print("call_sync  === recv data : ", dict_data)
        check_response(dict_data)
        return request.json_parser(dict_data)
